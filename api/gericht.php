...
if ($method === 'PUT') {
    ...
    if ($zutaten !== null) {
        $db->prepare("DELETE FROM gericht_zutaten WHERE gericht_id=?")->bind_param('i',$id) && true;
        ...
        foreach ($zutaten as $z) {
            ...
            $ft = (float)($z['fett_100g'] ?? 0);
            $kh = (float)($z['kh_100g'] ?? 0);
            $sz->bind_param('iisddddd', $id, $pid, $znam, $mg, $kc, $ew, $ft, $kh);
            $sz->execute();
        }
    }
